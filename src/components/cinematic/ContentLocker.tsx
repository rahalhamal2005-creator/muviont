...
export default function ContentLocker({ onUnlock, title, backdropUrl }: ContentLockerProps) {
  ...
  return (
    <div 
      className="w-full relative rounded-2xl flex flex-col items-center justify-center p-2 sm:p-8 md:p-12 text-center overflow-hidden border border-white/[0.06] bg-neutral-950 aspect-video min-h-[320px] sm:min-h-[360px]"
    >
      ...
      <div className="relative z-10 max-w-lg w-full bg-[#0c0c18]/85 border border-white/[0.06] rounded-2xl p-3.5 sm:p-7 backdrop-blur-xl shadow-2xl flex flex-col items-center justify-between gap-2.5 sm:gap-4">
        
        {/* Header: Transparent MUVIONT Logo (Hidden on mobile to save vertical space) */}
        ...
        <div className="space-y-0.5 sm:space-y-1">
          ...
          {title && (
            <h4 className="text-[10px] sm:text-sm font-bold text-neutral-300 line-clamp-1 italic px-2">
              &ldquo;{title}&rdquo;
            </h4>
          )}
        </div>

        {/* 3-Step Visual Conversion Timeline (Only show on tablet/desktop to save mobile space) */}
        <div className="hidden sm:grid w-full grid-cols-3 gap-1.5 border-y border-white/[0.04] py-3 text-center select-none">
        ...
        {/* Custom description with bold highlighted marketing hooks (Slightly smaller text on mobile to prevent clipping) */}
        <p className="text-[8.5px] sm:text-xs text-neutral-400 font-medium leading-relaxed max-w-md">
        ...
        {loading ? (
          ...
          <div className="w-full p-2 border border-white/[0.04] rounded-xl bg-white/[0.01]">
            <span className="text-[9px] text-neutral-500">No verification options available right now.</span>
            <button 
              onClick={onUnlock}
              className="mt-2 block w-full py-1.5 bg-neutral-800 hover:bg-neutral-700 text-[9px] font-black uppercase tracking-wider rounded-lg transition-colors"
            >
              Skip and Play
            </button>
          </div>
          ...
          <div className="w-full space-y-1.5">
            {offers.map((offer) => (
              ...
              >
                ...
                <span className="text-[8px] sm:text-[9px] font-black uppercase text-[var(--red)] tracking-wider shrink-0 ml-2 flex items-center gap-1">
        ...
        <div className="w-full mt-1 flex flex-col items-center justify-center gap-3 border-t border-white/[0.04] pt-2.5 sm:pt-4">
          <div className="flex items-center gap-2 justify-center">
            ...
            <span className="text-[8px] sm:text-[10px] font-black text-neutral-300 uppercase tracking-widest leading-none">
              {verifying 
